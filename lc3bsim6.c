...
/* Do not modify the above code.
   ...
   RUN_BIT
   RSystems/
(base) steffen@steffen-ubuntu:~/Documents/Embedded_Systems$ ls
embedded_final_project  homework2  ithemal
(base) steffen@steffen-ubuntu:~/Documents/Embedded_Systems$ cd ithemal/data_collection/
(base) steffen@steffen-ubuntu:~/Documents/Embedded_Systems/itEGS
   MEMORY
...
int bit(int num, int bitNum){
        return (num >> bitNum) & 0x00000001;
}
...
int sext(int num, int length){
        int signBit = bit(num, length-1);
        if (signBit) {
                return (num | (0xFFFFFFFF << length));
        }
        else {
                return num;
        }
...
int dependency_logic(int v, int sr1_needed, int sr2_needed, int sr1, int sr2, int de_br_op, int agex_ld_cc, 
	...
	//if data in DE is invalid, then we dont have a dependency
	if(v){
		//if instruction is a branch and an instruction further in the pipeline wants to set the condition codes,
		//we have a dependency	
		if(de_br_op){
			if(agex_ld_cc || mem_ld_cc || sr_ld_cc){
				return 1;
			}
		}
		
		//checks dependencies between source registers and destination registers in instructions further in the pipeline
		if(sr1_needed){
			if(agex_ld_reg && (sr1 == agex_drid)){
				return 1;
			}
			if(mem_ld_reg && (sr1 == mem_drid)){
				return 1;
			}
			if(sr_ld_reg && (sr1 == sr_drid)){
				return 1;
			}
		}
		if(sr2_needed){
			if(agex_ld_reg && (sr2 == agex_drid)){
				return 1;
			}
			if(mem_ld_reg && (sr2 == mem_drid)){
				return 1;
			}
			if(sr_ld_reg && (sr2 == sr_drid)){
				return 1;
			}
		}
...
/*dep_stall = dependency_logic(PS.DE_V, sr1_needed, sr2_needed, sr1, sr2, de_br_op,
                        v_agex_ld_cc, v_mem_ld_cc, v_sr_ld_cc, v_agex_ld_reg, v_mem_ld_reg,
...
int eval_agexaddr(){
	...
	int addr2mux = Get_ADDR2MUX(PS.AGEX_CS);
		...
		addr2mux_out = sext(bits(PS.AGEX_IR, 5, 0), 6);
	}else if(addr2mux == 2){
		addr2mux_out = sext(bits(PS.AGEX_IR, 8, 0), 9);
	}else{
...
int eval_addressmux(){
	if(Get_ADDRESSMUX(PS.AGEX_CS)){
		return eval_agexaddr();
	}
	return (bits(PS.AGEX_IR, 7, 0) << 1);
}
...
int eval_alu(){
	...
	if(Get_SR2MUX(PS.AGEX_CS)){
		b = sext((PS.AGEX_IR & 0x0000001F), 5);
	}else{
		b = PS.AGEX_SR2;
	}
...
int target_pc, trap_pc, mem_stall, mem_pcmux,
...
void MEM_stage() {
	...
	int we0, we1, data_size, mem_addr0;
	data_size = Get_DATA_SIZE(PS.MEM_CS);
	mem_addr0 = bit(PS.MEM_ADDRESS, 0);
...
void DE_stage() {
	...
	int de_br_op = Get_DE_BR_OP(csinst);	
	int ii, jj = 0;
